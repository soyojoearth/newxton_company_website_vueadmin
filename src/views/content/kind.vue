<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogVisible" width="30%" :before-close="handleClose" :modal-append-to-body="false">
...
export default {
  ...
  computed: {
    ListData() {
      return this.$store.state.new.CategoryListData
    }
  },
  watch: {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$confirm('确认删除？')
        .then(_ => {
          const then = this
          this.$store.dispatch('new/deleteCategory', row.category_id).then(() => {
            then.load()
            then.$message({
              type: 'success',
              message: '删除成功!'
    ...
    async UpdateCategory() {
      if (this.isCreate) {
        await this.$store.dispatch('new/createCategoryList', this.formLabelAlign)
        this.$message({
          type: 'success',
          message: '创建成功!'
        })
      } else {
        await this.$store.dispatch('new/updateCategory', this.formLabelAlign)
        this.$message({
          type: 'success',
          message: '修改成功!'
        })
